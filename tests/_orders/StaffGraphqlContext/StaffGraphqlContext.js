 ...
 *
 * The access-token rows carry NO explicit `id`, and that is deliberate.
 *
 * `staff_member_access_tokens` is a table the product itself writes, on the `signIn` and
 * `renewAccessToken` paths, where the id is auto-incremented. An auto-increment id is
 * `max(id) + 1`, so the moment this file inserted an explicit `10100401` the next access token
 * `signIn` minted anywhere in the run was assigned `10100402` — the very id this file's next case
 * wanted. `tests/_orders/` suites share one database and jest runs them concurrently, so the
 * collision landed on whichever case lost the race and the run failed with
 * `UNIQUE constraint failed: staff_member_access_tokens.id` on a row nothing else had written.
 *
 * No id block can fix that, which is the part worth keeping: the block's own first insert is what
 * hands the next auto-increment writer the block's second id. So the ids are left to the database,
 * and a case is told apart by its `access_token` — which is what the lookup uses anyway.
 *
 * The two `StaffMemberId`s naming nobody do keep explicit ids from this feature's block
 * (10100441, 10100442). `staff_members` is safe to write that way because every writer of it
 * supplies an explicit id: §4 rules sign-up out of scope, so nothing in the product inserts one.
 *
